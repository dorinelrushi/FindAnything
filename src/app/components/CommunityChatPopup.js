...
export default function CommunityChatPopup({ isOpen, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[998] flex items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm sm:items-end md:items-center">
            <div className="bg-surface w-full sm:max-w-5xl h-full sm:h-[85vh] md:h-[700px] sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row shadow-orange-500/10 transition-all duration-300">
                
                {/* Sidebar - Categories */}
                <div className={`
                    fixed md:relative inset-y-0 left-0 z-[1001] md:z-0
                    w-72 md:w-64 bg-bg-light border-r border-border-light flex flex-col pt-2 
                ...
                `}>
                    <div className="p-5 border-b border-border-light flex items-center justify-between">
                        <div>
                            <h2 className="text-lg font-bold text-text-primary leading-none">Community</h2>
                            <p className="text-[10px] text-text-secondary mt-2 font-medium uppercase tracking-wider">Select Category</p>
                        </div>
                        <button onClick={() => setIsSidebarOpen(false)} className="md:hidden p-2 hover:bg-border-light rounded-lg text-text-secondary transition-colors">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                ...
                <div className="flex-1 flex flex-col bg-surface overflow-hidden">
                    {/* Header */}
                    <div className="h-16 border-b border-border-light flex items-center justify-between px-4 sm:px-6 bg-surface/90 backdrop-blur-md sticky top-0 z-10 transition-all duration-300">
                        <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-text-primary text-sm sm:text-base tracking-tight">{CATEGORIES.find(c => c.id === activeCategory)?.label}</h3>
                                <div className="flex items-center gap-1.5 sm:gap-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.5)]"></span>
                                    <span className="text-[10px] text-text-secondary font-medium uppercase tracking-wider">Live Chat</span>
                                </div>
                            </div>
                        </div>
                        <button 
                            onClick={onClose} 
                            className="p-2.5 hover:bg-bg-light rounded-xl text-text-secondary hover:text-text-secondary transition-all active:scale-90"
                            title="Close Chat"
                        >
                    ...
                    <div ref={chatContainerRef} className="flex-1 p-4 md:p-6 overflow-y-auto space-y-6 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed">
                        {messages.length === 0 ? (
                            <div className="h-full flex flex-col justify-center items-center text-text-secondary space-y-4">
                                ...
                                return (
                                    <div key={msg._id} className={`group/msg flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                            ...
                                            {isMe && editingId !== msg._id && (
                                                <div className="flex items-center gap-1 opacity-0 group-hover/msg:opacity-100 transition-opacity">
                                                    <button
                                                        ...
                                                        className="p-1 hover:bg-border-light rounded-md text-text-secondary hover:text-text-secondary transition-colors"
                                                        title="Edit"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1 hover:bg-red-100 rounded-md text-text-secondary hover:text-red-500 transition-colors"
                                                        title="Delete"
                                                    >
                                            ...
                                            <div className="max-w-[85%] md:max-w-[70%] w-full">
                                                ...
                                                <div className="flex gap-2 mt-1.5 justify-end">
                                                    <button
                                                        ...
                                                        className="px-3 py-1 text-xs text-text-secondary hover:bg-bg-light rounded-lg transition-colors"
                                                    >
                                                        Cancel
                                                    </button>
                                                    <button
                                        ...
                                        <div className={`max-w-[85%] md:max-w-[70%] rounded-2xl p-4 shadow-sm ${
                                            isMe ? 'bg-orange-500 text-white rounded-tr-sm' : 'bg-bg-light text-text-primary rounded-tl-sm'
                                        }`}>
                                                ...
                                                <div className="mt-3 space-y-2 w-full">
                                                    {msg.attachments.map(att => (
                                                        <Link href={`/${att.type}/${att.slug}`} target="_blank" key={att._id} className={`group flex items-center justify-between gap-2 sm:gap-3 p-2 rounded-xl transition-all border shadow-sm ${
                                                            isMe ? 'bg-orange-600 border-orange-400 hover:bg-orange-700' : 'bg-surface border-border-light hover:border-border-light'
                                                        }`}>
                                                            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                                                                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-border-light flex-shrink-0 overflow-hidden shadow-inner">
                                                                    {att.image ? (
                                                                        ...
                                                                        <MapPin className="w-5 h-5 m-auto text-text-secondary" />
                                                                    )}
                                                                </div>
                                                                <div className="flex-1 min-w-0">
                                                                    <p className={`font-semibold text-[13px] sm:text-sm truncate ${isMe ? 'text-white' : 'text-text-primary'}`}>{att.title}</p>
                                                                    <p className={`text-[11px] truncate capitalize mt-0.5 ${isMe ? 'text-orange-200' : 'text-text-secondary'}`}>{att.type} • {att.city}</p>
                                                                </div>
                                                            </div>
                                                            <div className={`px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-semibold whitespace-nowrap transition-colors ${
                                                                isMe ? 'bg-orange-500/80 text-white group-hover:bg-orange-500' : 'bg-bg-light text-text-primary group-hover:bg-border-light'
                                                            }`}>
                                                                View
                                                            </div>
                                                        </Link>
                    ...
                    <div className="p-4 bg-surface border-t border-border-light">
                        ...
                        <div className="flex items-end gap-2 bg-bg-light border border-border-light rounded-2xl p-2 focus-within:ring-2 focus-within:ring-orange-500/20 focus-within:border-orange-500 transition-all">
                            <button 
                                onClick={() => setIsPickerOpen(true)}
                                className="p-3 text-text-secondary hover:text-orange-500 hover:bg-orange-50 rounded-xl transition-colors flex-shrink-0"
                            ...
                            <textarea
                                ...
                                className="flex-1 bg-transparent border-none focus:ring-0 p-3 max-h-32 min-h-[44px] resize-none text-sm text-text-primary"
                                rows={1}
                            />
            ...
            {showNameModal && (
                <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-surface rounded-2xl p-6 shadow-2xl max-w-sm w-full animate-in zoom-in-95">
                        <h3 className="text-lg font-bold text-text-primary mb-2">Join the Chat</h3>
                        <p className="text-sm text-text-secondary mb-5">Please enter a display name to start chatting and sending suggestions.</p>
                        <input
                            type="text"
                            ...
                            className="w-full px-4 py-3 bg-bg-light border border-border-light rounded-xl mb-4 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all"
                            autoFocus
                        />
                        <div className="flex justify-end gap-3">
                            <button
                                onClick={() => setShowNameModal(false)}
                                className="px-5 py-2.5 text-sm font-medium text-text-secondary hover:bg-bg-light rounded-xl transition-colors"
                            >
                                Cancel
                            </button>
                            <button
