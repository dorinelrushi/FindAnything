        ...
        <div className="container-wide max-w-[900px] mt-10 mb-20 bg-white p-8 md:p-12 rounded-3xl shadow-airbnb border border-border-light">
            <div className="flex justify-between items-center mb-8 pb-6 border-b border-border-light">
                ...
                <div className="flex gap-4">
                    <Link href={`/blog/${originalSlug}`} className="text-text-secondary hover:text-brand font-bold transition-colors">👁️ View</Link>
                    <Link href="/admin" className="text-text-secondary hover:text-brand font-bold transition-colors">← Admin</Link>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                ...
                <div>
                    ...
                    <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Blog Title" className="input font-bold text-lg" required />
                </div>
                ...
                <div>
                    ...
                    {showImageDialog && (
                        <div className="p-4 bg-brand/5 border border-brand/20 border-b-0 flex gap-3 items-center">
                            <input autoFocus value={imageUrlInput} onChange={e => setImageUrlInput(e.target.value)} placeholder="Paste image URL here..." className="input flex-1 m-0" onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); insertImage(); } if (e.key === 'Escape') setShowImageDialog(false); }} />
                ...
                <div className="bg-bg-light p-6 rounded-2xl border border-border-light space-y-4">
                    ...
                    <div className="flex flex-col gap-4">
                        <div>
                            <label className="block mb-2 text-text-secondary font-bold text-sm">SEO Title</label>
                            <input value={seoTitle} onChange={e => setSeoTitle(e.target.value)} placeholder={title} className="input" />
                        </div>
                        <div>
                            <label className="block mb-2 text-text-secondary font-bold text-sm">SEO Description (max 160 chars)</label>
                            <textarea value={seoDescription} onChange={e => setSeoDescription(e.target.value)} placeholder="Description shown in Google..." className="input resize-y min-h-[100px]" maxLength={160} />
            ...
            <style jsx>{`
                [contenteditable]:empty:before { content: attr(data-placeholder); color: rgba(0,0,0,0.3); pointer-events: none; }
